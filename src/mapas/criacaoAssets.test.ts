...
describe('manifesto de assets da Criação', () => {
  ...
  it('expõe lookup estável por ID para o loader futuro', () => {
    ...
    expect(adao?.arquivo).toBe('/models/creation/characters/adam.model.json')
    expect(eva?.arquivo).toBe('/models/creation/characters/eve.model.json')
    expect(ovelha?.licenca.identificador).toBe('CC0-1.0')
    expect(adao?.licenca.identificador).toBe('Project-authored')
    expect(eva?.licenca.identificador).toBe('Project-authored')
